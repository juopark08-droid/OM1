...
1. Interface (`interface.py`): Defines the input and output types.
2. Implementation (`implementation/`): Contains business logic. If no logic is required, use `passthrough`.
3. Connector (`connector/`): Connects OM1 to specific virtual or physical environments, typically through middleware (e.g. custom APIs, `ROS2`, `Zenoh`, or `CycloneDDS`)

```tree
actions/
└── move_{unique_hardware_id}/
    ├── interface.py      # Defines MoveInput/MoveOutput
    ├── implementation/
    │   └── passthrough.py
    └── connector/
...
In general, each robot has its own set of capabilities, so each action is hardware-specific.

For example, if you are adding support for the Unitree G1 Humanoid (version 13.2b), which introduces a new movement subtype such as `dance_2`, you could name the updated action:

    move_unitree_g1_13_2b

and reference that action in your `unitree_g1.json` configuration file.
